...
price_df = pd.read_sql_query("SELECT * FROM ohlcv_5m ORDER BY timestamp DESC LIMIT 47000", conn)
indicators_df = pd.read_sql_query("SELECT * FROM indicators_5m ORDER BY timestamp DESC LIMIT 47000", conn)
...
for epoch in range(epochs):
    ...
    print(f"Epoch {epoch+1}/{epochs} - Train Loss: {avg_train_loss:.6f} - Validation Loss: {avg_val_loss:.10f}")
...
print(f"5 dakika sonrası tahmini fiyat: {predicted_prices[0]:.2f}")
print(f"15 dakika sonrası tahmini fiyat: {predicted_prices[1]:.2f}")
print(f"1 saat sonrası tahmini fiyat: {predicted_prices[2]:.2f}")
